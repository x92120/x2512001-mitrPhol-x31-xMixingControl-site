...
def clear_recipe_in_plc(plant_id: int = Path(..., title="Plant ID (1, 2, or 3)")):
    ...
    # Clear telemetry and overview cache
    if plant_id in _telem_cache:
        _telem_cache.pop(plant_id, None)
    global _recipe_target_cache, _plants_overview_cache
    _recipe_target_cache.pop(plant_id, None)
    _plants_overview_cache.clear()
...
def complete_batch_and_release_plant(
    ...
    # 6. Clear Telemetry & Overview Cache
    if plant_id in _telem_cache:
        _telem_cache.pop(plant_id, None)
    global _recipe_target_cache, _plants_overview_cache
    _recipe_target_cache.pop(plant_id, None)
