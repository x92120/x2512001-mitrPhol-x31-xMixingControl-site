...
const resolvePhaseType = (step: any): number => {
    ...
    const pNum = String(step.phase_number || step.phase_id || '').toLowerCase().replace(/[^0-9]/g, '')
    const pNumVal = parseInt(pNum, 10)
    const last2 = pNum.slice(-2)
    const last2Num = parseInt(last2, 10)

    const pId = String(step.phase_id || '').toUpperCase()
    const pNumStr = String(step.phase_number || '').toUpperCase()
    const desc = String(step.description || step.action_description || step.action || '').toUpperCase()
    const combined = `${pId} ${pNumStr} ${desc}`

    // 1. Direct standard phase IDs & keywords
    ...
    // 2. Action Code deduction (Robust fallback)
    const ac = Number(step.action_code || 0)
    const st = Number(step.step_time || 0)
    const ts = Number(step.temperature || step.temp_sp || 0)

    if ([10010, 10020, 10030, 10040].includes(ac)) return 1

    if (ac === 30500) {
      if (st > 0 && ts >= 83.0 && (combined.includes('PAST') || last2Num === 50 || pNumVal === 130)) return 6
      if (combined.includes('HOLD') || last2Num === 60 || pNumVal === 140) return 7
      return 5 // Standard Heating (p040..p049, p090, p120)
    }

    if (ac === 30600) {
      if (combined.includes('TRANS') || last2Num === 70 || pNumVal >= 150) return 8
      return 7 // Cooling in tank
    }

    // 3. Match by Phase Number
    if (last2 === '10' || pNumVal === 10 || pNumVal === 110) return 1
    if (last2 === '20' || pNumVal === 20 || (pNumVal === 120 && combined.includes('SHEAR'))) return 2
    if (last2 === '30' || pNumVal === 30) return 3
    if (last2 === '35' || last2 === '36' || pNumVal === 35) return 4
    if ((last2Num >= 40 && last2Num <= 49) || (pNumVal >= 40 && pNumVal <= 49) || (pNumVal >= 140 && pNumVal <= 149) || pNumVal === 90 || pNumVal === 120) return 5
    if (last2 === '50' || pNumVal === 50 || pNumVal === 130) return 6
    if (last2 === '60' || pNumVal === 60 || pNumVal === 140) return 7
    if (last2 === '70' || pNumVal === 70 || pNumVal >= 150) return 8
...
const fetchStampTimes = async (batchId: string) => {
    ...
    try {
        ...
        // Restore scannedVolumeMap from stamped logs

        const restoredMap: Record<string, number> = { ...scannedVolumeMap.value }

        for (const s of skuSteps.value) {

            if ((s.stamp_time != null && s.stamp_time !== '-') || (s.actual_volume != null && Number(s.actual_volume) > 0)) {

                const pKey = `${s.phase_number}|${s.re_code}`

                const val = s.actual_volume != null ? Number(s.actual_volume) : (productionRequire(s) || 1)

                restoredMap[pKey] = val

                if (s.re_code) restoredMap[s.re_code] = val

            }

        }

        scannedVolumeMap.value = restoredMap
...
const isStepAllGreen = (step: any): { ok: boolean; failed: string[] } => {
    ...
    const rcLower = (step.re_code || '').toLowerCase()

    const isExcludedRe = rcLower.includes('ro-water') || rcLower.includes('ro water') || rcLower.includes('liquid sugar') || rcLower.includes('ls in line')

    const stepIsScanning = (aCode.startsWith('2') || aCode.startsWith('3')) && hasReCode && !isExcludedRe && productionRequire(step) > 0



    const isStepDone = (s: any) => {
        const phaseScanKey = `${s.phase_number}|${s.re_code}`
        const reqVol = productionRequire(s)
        const scannedVol = scannedVolumeMap.value[phaseScanKey] ?? scannedVolumeMap.value[s.re_code]
        if (scannedVol != null) {
            if (reqVol <= 0) return true
            const tolLow = Number(s.low_tol || (reqVol * 0.02))
            if (scannedVol >= (reqVol - tolLow) || isWeightInTolerance(s, scannedVol)) return true
        }
        return (s.stamp_time != null && s.stamp_time !== '-') ||
               (s.actual_volume != null && Number(s.actual_volume) > 0)
    }
    ...
    if (isFree) {
        ...
        const allFreeScan = skuSteps.value.filter((s: any) => {
            ...
            const saCode = String(s.action_code || '')

            if (!saCode.startsWith('2') && !saCode.startsWith('3')) return false

            if (!s.re_code || s.re_code === '-' || !s.re_code.trim()) return false

            const srcLower = s.re_code.toLowerCase()

            if (srcLower.includes('ro-water') || srcLower.includes('ro water') || srcLower.includes('liquid sugar') || srcLower.includes('ls in line')) return false

            return productionRequire(s) > 0

        })



        if (allFreeScan.length > 0 && stepIsScanning) {

            const scannedCount = allFreeScan.filter(isStepDone).length
        ...
        // Standard (non-free-scan) step

        if (stepIsScanning && !isStepDone(step)) {
...
const activeFreeScanPhaseGroup = computed(() => {
    ...
    const scannedSteps = phaseScanSteps.filter(isStepDone)

    const pendingSteps = phaseScanSteps.filter(s => !isStepDone(s))
...
const scannedBagsMap = ref<Record<string, Record<string, number>>>({}) // multi-bag accumulation: scanKey -> { [barcodeId]: netWeight }
...
/** Fuzzy/prefix matcher connecting truncated PLC re_code names (e.g. "Frozen  Lemon Concen")

 * with full Prebatch master names (e.g. "Frozen  Lemon Concentrate") */
...
const findPrebatchKey = (rc: string): string => {
    ...
    // 3. Alphanumeric normalized exact match
    ...
    // 4. Prefix / Substring match (handles truncated strings like "Frozen Tangerine Con" vs "Frozen Tangerine Concentrate")
...
const getStepPrebatchId = (step: any): string => {
    ...
    if (key && prebatchIdMap.value[key]) return prebatchIdMap.value[key]

    return prebatchIdMap.value[rc] || '-'

}



const getStepPrebatchWeight = (step: any): number | null => {
    ...
    if (key && prebatchWeightMap.value[key] != null) return prebatchWeightMap.value[key]

    return prebatchWeightMap.value[rc] ?? null
...
const fetchMultiPlantSummary = async () => {
    ...
    try {
        ...
        const plantsDict = allPlantsRes?.plants || (allPlantsRes?.['1'] || allPlantsRes?.['2'] || allPlantsRes?.['3'] ? allPlantsRes : (allPlantsRes || {}))
                ...
                if (isCurrentActivePlant) {
                    ...
                    if (currentPhaseScanSteps.length > 0) {
                        ...
                        const pendingInCurrentPhase = currentPhaseScanSteps.filter((s: any) => {
                            ...
                            const isScanned = scannedVolumeMap.value[phaseScanKey] != null ||

                                              scannedVolumeMap.value[rawKey] != null ||

                                              (s.stamp_time != null && s.stamp_time !== '-') ||
...
// ── Multi-Bag Accumulation Engine ──
interface BagAccumResult {
    isDuplicate: boolean
    isStepComplete: boolean
    accumulatedVol: number
    bagNetWeight: number
    bagProgressStr: string
    bagCount: number
}

const processBagAccumulation = (
    step: any,
    barcodeId: string,
    qrData: any,
    reqVol: number
    ...
    if (!scannedBagsMap.value[scanKey]) {
        scannedBagsMap.value[scanKey] = {}
    }
    const existingBags = scannedBagsMap.value[scanKey]

    // 1. Determine net weight of this bag
    let bagNetWeight = qrData?.n ?? qrData?.r ?? qrData?.v ?? qrData?.w ?? qrData?.qty ?? qrData?.weight ?? qrData?.volume ?? qrData?.actual ?? null
    if (bagNetWeight == null || isNaN(Number(bagNetWeight)) || Number(bagNetWeight) <= 0) {
        const totalQrWeight = qrData?.t ?? qrData?.req
        if (totalQrWeight != null && Number(totalQrWeight) > 0) {
            bagNetWeight = Number(totalQrWeight)
        } else {
            const preVol = prebatchWeightMap.value[step.re_code]
            bagNetWeight = (preVol != null && preVol > 0) ? preVol : (reqVol > 0 ? reqVol : 1)
        }
    } else {
        bagNetWeight = Number(bagNetWeight)
    }

    // 2. Check duplicate scan
    if (existingBags[barcodeId] != null) {
        const currentAccum = Object.values(existingBags).reduce((sum, v) => sum + v, 0)
        return {
            isDuplicate: true,
            isStepComplete: isWeightInTolerance(step, currentAccum) || (currentAccum >= (reqVol - Number(step.low_tol || (reqVol * 0.02)))),
            accumulatedVol: currentAccum,
            bagNetWeight,
            bagProgressStr: qrData?.p ? `ถุงที่ ${qrData.p}` : `ถุง #${Object.keys(existingBags).length}`,
            bagCount: Object.keys(existingBags).length
        }
    }

    // 3. Add new bag
    existingBags[barcodeId] = bagNetWeight
    const accumulatedVol = Object.values(existingBags).reduce((sum, v) => sum + v, 0)
    scannedBagsMap.value = { ...scannedBagsMap.value, [scanKey]: { ...existingBags } }
    scannedVolumeMap.value = { ...scannedVolumeMap.value, [scanKey]: accumulatedVol }
    prebatchWeightMap.value = { ...prebatchWeightMap.value, [step.re_code]: accumulatedVol }

    const tolLow = Number(step.low_tol || (reqVol * 0.02))
    const isStepComplete = (reqVol <= 0) || (accumulatedVol >= (reqVol - tolLow)) || isWeightInTolerance(step, accumulatedVol)

    const bagCount = Object.keys(existingBags).length
    const bagProgressStr = qrData?.p ? `ถุงที่ ${qrData.p}` : `ถุง #${bagCount}`
...
const handleScan = (scannedText: string) => {
    ...
    for (const step of skuSteps.value) {
        ...
        if (isExactMatch || isNameMatch) {
            ...
            if (wrongPhase) {
                ...
                if (activeFreeScanPhase) {
                    ...
                    if (sameReInActivePhase) {
                        ...
                        const accum2 = processBagAccumulation(redirectedStep, barcodeId, qrData, reqVol2)

                        if (accum2.isDuplicate) {
                            playSweetVoice('alert')
                            $q.notify({
                                type: 'warning',
                                icon: 'repeat',
                                message: isThai.value ? `⚠️ ถุงนี้ (${accum2.bagProgressStr}) สแกนบันทึกไปแล้ว` : `⚠️ Bag (${accum2.bagProgressStr}) already scanned`,
                                caption: isThai.value ? `ยอดสะสมปัจจุบัน: ${accum2.accumulatedVol.toFixed(2)} / ${reqVol2.toFixed(2)} kg` : `Current total: ${accum2.accumulatedVol.toFixed(2)} / ${reqVol2.toFixed(2)} kg`,
                                position: 'top',
                                timeout: 3000
                            })
                            return
                        ...
                        if (!accum2.isStepComplete) {
                            ...
                            $q.notify({
                                ...
                                message: isThai.value 
                                    ? `📦 สแกนสะสม: ${redirectedStep.re_code} (${accum2.bagProgressStr}) +${accum2.bagNetWeight.toFixed(2)} kg` 
                                    : `📦 Bag scanned: ${redirectedStep.re_code} (${accum2.bagProgressStr}) +${accum2.bagNetWeight.toFixed(2)} kg`,
                                caption: isThai.value 
                                    ? `ยอดสะสม: ${accum2.accumulatedVol.toFixed(2)} / ${reqVol2.toFixed(2)} kg (${((accum2.accumulatedVol/reqVol2)*100).toFixed(1)}%) — กรุณาสแกนถุงที่เหลือต่อ` 
                                    : `Accumulated: ${accum2.accumulatedVol.toFixed(2)} / ${reqVol2.toFixed(2)} kg — please scan remaining bags`,
                                position: 'top',
                                timeout: 3500
                            })
                            return
                        ...
                        const scannedVol2 = accum2.accumulatedVol
                        ...
                        const allFreeScanSteps2 = skuSteps.value.filter((s: any) => {
                            ...
                            if (s.phase_number !== matchedPhase2) return false
                        ...
                        const isFreeStepDone2 = (s: any) => {
                            const phaseScanKey2 = `${s.phase_number}|${s.re_code}`
                            const reqVol = productionRequire(s)
                            const scannedVol = scannedVolumeMap.value[phaseScanKey2] ?? scannedVolumeMap.value[s.re_code]
                            if (scannedVol != null) {
                                if (reqVol <= 0) return true
                                const tolLow = Number(s.low_tol || (reqVol * 0.02))
                                if (scannedVol >= (reqVol - tolLow) || isWeightInTolerance(s, scannedVol)) return true
                        ...
                        const scannedCount2 = allFreeScanSteps2.filter(isFreeStepDone2).length
            ...
            const accum = processBagAccumulation(step, barcodeId, qrData, reqVol)

            if (accum.isDuplicate) {
                playSweetVoice('alert')
                $q.notify({
                    type: 'warning',
                    icon: 'repeat',
                    message: isThai.value ? `⚠️ ถุงนี้ (${accum.bagProgressStr}) สแกนบันทึกไปแล้ว` : `⚠️ Bag (${accum.bagProgressStr}) already scanned`,
                    caption: isThai.value ? `ยอดสะสมปัจจุบัน: ${accum.accumulatedVol.toFixed(2)} / ${reqVol.toFixed(2)} kg` : `Current total: ${accum.accumulatedVol.toFixed(2)} / ${reqVol.toFixed(2)} kg`,
                    position: 'top',
                    timeout: 3000
                })
                return
            ...
            if (!accum.isStepComplete) {
                ...
                $q.notify({
                    ...
                    icon: 'hourglass_top',
                    message: isThai.value 
                        ? `📦 สแกนสะสม: ${step.re_code} (${accum.bagProgressStr}) +${accum.bagNetWeight.toFixed(2)} kg` 
                        : `📦 Bag scanned: ${step.re_code} (${accum.bagProgressStr}) +${accum.bagNetWeight.toFixed(2)} kg`,
                    caption: isThai.value 
                        ? `ยอดสะสม: ${accum.accumulatedVol.toFixed(2)} / ${reqVol.toFixed(2)} kg (${((accum.accumulatedVol/reqVol)*100).toFixed(1)}%) — กรุณาสแกนถุงที่เหลือต่อ` 
                        : `Accumulated: ${accum.accumulatedVol.toFixed(2)} / ${reqVol.toFixed(2)} kg — please scan remaining bags`,
                    position: 'top',
                    timeout: 3500
                })
                return
            ...
            const scannedVol = accum.accumulatedVol
            const scanKey = `${step.phase_number}|${step.re_code}`
            ...
            // 2. All manual-scan steps in this phase
            ...
            const scannedCount = allFreeScanSteps.filter(isFreeStepDone).length
            ...
            if (allScanned) {
                ...
                try {
                    ...
                    $fetch(`${appConfig.apiBaseUrl}/plc/plant/${activePlantId.value || 1}/step-complete`, {
                    ...
                    console.log(`[FreeScan] Triggered Step_complete for plant ${activePlantId.value || 1}`)
    ...
    if (matchedStep) {
        ...
        const accum = processBagAccumulation(matchedStep, barcodeId, qrData, reqVol)

        if ((whType === 'FH' || whType === 'SPP')) {
            if (accum.isDuplicate) {
                playSweetVoice('alert')
                $q.notify({
                    type: 'warning',
            ...
            if (!accum.isStepComplete) {
                ...
                $q.notify({
                    ...
                    icon: 'hourglass_top',
                    message: isThai.value 
                        ? `📦 สแกนสะสม: ${matchedStep.re_code} (${accum.bagProgressStr}) +${accum.bagNetWeight.toFixed(2)} kg` 
                        : `📦 Bag scanned: ${matchedStep.re_code} (${accum.bagProgressStr}) +${accum.bagNetWeight.toFixed(2)} kg`,
                    caption: isThai.value 
                        ? `ยอดสะสม: ${accum.accumulatedVol.toFixed(2)} / ${reqVol.toFixed(2)} kg (${((accum.accumulatedVol/reqVol)*100).toFixed(1)}%) — กรุณาสแกนถุงที่เหลือต่อ` 
                        : `Accumulated: ${accum.accumulatedVol.toFixed(2)} / ${reqVol.toFixed(2)} kg — please scan remaining bags`,
                    position: 'top',
            ...
            $q.notify({
                ...
                caption: `Volume: ${scannedVol.toFixed(2)} / ${reqVol.toFixed(2)} kg | Bag: ${barcodeId}`,
                position: 'top',
                icon: 'check_circle',
                timeout: 3000
            })
                ...
                <template v-for="phaseGroup in skuStepsByPhase" :key="phaseGroup.phase">
                  ...
                  <template v-for="step in phaseGroup.steps" :key="step.id">
                    ...
                    <tr v-show="isPhaseExpanded(phaseGroup.phase)"
                      ...
                      <td class="text-caption text-grey-8" style="font-family: monospace;">{{ getStepPrebatchId(step) }}</td>
                      ...
                      <td class="text-right">
                        ...
                        <template v-if="currentStep && (step.id === currentStep.id || (step.phase_number === currentStep.phase_number && step.sub_step === currentStep.sub_step))">
                          ...
                          <!-- Scan step with confirmed scan: show scan volume, not load cell -->

                          <template v-if="((getStepWh(step) === 'SPP' || getStepWh(step) === 'FH') || (String(step.action_code || '').startsWith('2') || String(step.action_code || '').startsWith('3'))) && (scannedVolumeMap[`${step.phase_number}|${step.re_code}`] != null || scannedVolumeMap[step.re_code] != null || (step.stamp_time != null && step.stamp_time !== '-'))">

                            <span class="act-num text-green-8" title="Volume confirmed from scan">✔ {{ Number(scannedVolumeMap[`${step.phase_number}|${step.re_code}`] || scannedVolumeMap[step.re_code] || step.actual_volume || productionRequire(step)).toFixed(2) }}</span>
        ...
        <q-card-section class="bg-blue-9 text-white row items-center q-pb-sm">
          ...
          <div>
            ...
            <div class="text-caption" style="opacity:0.85;">
              ...
              {{ qrScanStep ? `${getStepWh(qrScanStep) || 'WH'} — ${qrScanStep.re_code}` : '' }}
