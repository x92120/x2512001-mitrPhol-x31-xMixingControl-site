...
const handleGlobalScanKeydown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'Enter' || e.key === 'Tab') {
        const isFastBurst = scannerCharDeltas.length >= 3 && (scannerCharDeltas.reduce((a, b) => a + b, 0) / scannerCharDeltas.length) < 70
        const isRecognizedBarcode = globalScannerBuffer.startsWith('{') || 
                                    globalScannerBuffer.toUpperCase().startsWith('P') || 
                                    globalScannerBuffer.toUpperCase().startsWith('IND') ||
                                    globalScannerBuffer.includes('-') ||
                                    globalScannerBuffer.length >= 6

        // If user is typing in a search box manually and it is NOT a scanner burst, let them submit normally
        if (isOtherInputField && !isFastBurst && !isRecognizedBarcode && globalScannerBuffer.length < 4) return
        ...
        if (codeToProcess) {
            ...
            // Clean up any characters typed into the active input during the fast barcode scan
            if (isOtherInputField && activeEl) {
                try {
        ...
        if (delta <= 130) {
            scannerCharDeltas.push(delta)
        }
        globalScannerBuffer += e.key

        // If fast scanner burst is detected (3+ chars under 65ms), preventDefault on other input fields to stop pollution
        if (isOtherInputField && scannerCharDeltas.length >= 2) {
            const avgDelta = scannerCharDeltas.reduce((a, b) => a + b, 0) / scannerCharDeltas.length
            if (avgDelta < 65) {
                e.preventDefault()
                e.stopPropagation()
            }
