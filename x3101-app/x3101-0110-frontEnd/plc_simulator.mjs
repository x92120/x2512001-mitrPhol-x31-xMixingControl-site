...
client.on('connect', () => {
    ...
    // Subscribe to commands coming from the Web UI x61
    ...
    setInterval(() => {
        ...
        // Note: useMQTT.ts in x61 filters automatically if topic includes "plant/1"
...
client.on('message', (topic, message) => {
    ...
    if (cmd === 'NEXT_STEP') {
        ...
        console.log('▶️ PLC STARTED!')
    } else if (cmd === 'PAUSE') {
        console.log('⏸️ PLC PAUSED!')
    } else if (cmd === 'ABORT') {
        console.log('🛑 PLC EMERGENCY STOPPING! (ABORT)')
    }
})
