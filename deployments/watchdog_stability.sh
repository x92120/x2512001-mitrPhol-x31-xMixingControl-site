#!/bin/bash
# =============================================================================
# xMixing Control - Industrial Stability & Health Watchdog (Production Systemd)
# =============================================================================
...
if [ "$FASTAPI_STATUS" != "200" ]; then
    log_msg "[WARNING] FastAPI Backend (Port 8031) not responding (status: $FASTAPI_STATUS). Restarting via systemd..."
    systemctl --user restart xmixing-backend.service 2>/dev/null || {
...
if [ "$NUXT_STATUS" != "200" ] && [ "$NUXT_STATUS" != "302" ]; then
    log_msg "[WARNING] Nuxt Frontend (Port 3031) not responding (status: $NUXT_STATUS). Restarting via systemd..."
    systemctl --user restart xmixing-frontend.service 2>/dev/null || {
...
# 3. MEMORY LEAK GUARD (> 600MB for Node)
NODE_PID=$(pgrep -f ".output/server/index.mjs" | head -n 1)
if [ -n "$NODE_PID" ]; then
    NODE_MEM_KB=$(ps -p "$NODE_PID" -o rss= 2>/dev/null | tr -d ' ')
    if [ -n "$NODE_MEM_KB" ] && [ "$NODE_MEM_KB" -gt 600000 ]; then
        log_msg "[INFO] Nuxt Memory exceeded 600MB ($NODE_MEM_KB KB). Graceful recycling..."
        systemctl --user restart xmixing-frontend.service
    fi
fi
